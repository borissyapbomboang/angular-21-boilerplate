...
- App initializer (calls refresh on first app load):
...
2. The API returns a response that includes a `jwtToken`
...
Root code lives under `src/app`:

- `_services/` shared services (e.g. `AccountService`, `AlertService`)
- `_helpers/` shared helpers (guards, interceptors, app initializer, fake backend)
- `_models/` shared types and enums (Account, Role, Alert)
- `account/` auth screens (login/register/forgot, etc.)
- `admin/` admin screens for account management

The UI is styled with Bootstrap 5 via a CDN link in `index.html`.
...
If you are using a real API, make sure it sets a refresh token cookie and supports `POST /accounts/refresh-token`. Also make sure you configure CORS to allow credentials and ensure cookies are set with the correct `SameSite` / `Secure` settings.
